...
const ALLOWED_REPORT_TYPES = ["group", "member", "audit"];
const MAX_PAYLOAD_BYTES = 100_000; // ~100 KB cap to limit prompt-injection surface
...
serve(async (req) => {
  ...
  try {
    ...
    const { reportType, data } = JSON.parse(rawBody || "{}");
